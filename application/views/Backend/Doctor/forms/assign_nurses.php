...
<div style="margin:10px;">
    <a href="<?= base_url('index.php/Doctor/assign_nurses_list') ?>">
...
<div class="row animated fadeInDown">
    <div class="col-lg-10 col-md-10 col-xs-10 col-xs-offset-1 col-md-offset-1 col-lg-offset-1">
        <form class="form-horizontal" action="<?= site_url('index.php/doctor/manage_assign_nurses'); ?>" method="post">
            
            <div class="form-group"><label class="col-lg-2 control-label">Select Patient</label>
                <div class="col-lg-10">
                    <select data-placeholder="Choose a Country..." class="chosen-select" name="patient_id" required
                        autocomplete="off">
                        <option value="0">Select Patient</option>
                        <?php foreach($records->patients as $patient) : ?>
                        <option value="<?= $patient->id; ?>"
                            <?php echo ($patient->id==$records->patient_id) ? 'selected' : '' ?>><?= $patient->name; ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            <div class="form-group"><label class="col-lg-2 control-label">Select Nurse</label>
                <div class="col-lg-10">
                    <select data-placeholder="Choose a Country..." class="chosen-select" name="nurse_id" required
                        autocomplete="off">
                        <option value="0">Select Nurse</option>
                        <?php foreach($records->nurses as $nurse) : ?>
                        <option value="<?= $nurse->id; ?>"
                            <?php echo ($nurse->id==$records->nurse_id) ? 'selected' : '' ?>><?= $nurse->name; ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="col-lg-10">
                    <div class="input-group date">
                        <span class="input-group-addon"><i class="fa fa-calendar"></i></span><input type="text"
                            name="date" required="required" autocomplete="off" class="form-control"
                            value="<?= isset($_POST['date']) ? $_POST['date'] : $records->date; ?>">
                    </div>
                </div>
            </div>
            ...
            <div class="form-group" id="data_1">
                ...
                <div class="col-lg-10">
                    <div class="input-group time">
                        <input type="time" name="time" autocomplete="off" required="required" class="form-control"
                            value="<?= isset($_POST['time']) ? $_POST['time'] : $records->time; ?>">
