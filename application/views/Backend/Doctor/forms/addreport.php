...
<div style="margin:10px;">
    <a href="<?= base_url('index.php/Doctor/report') ?>">
...
<div class="row animated fadeInDown">
    <div class="col-lg-10 col-md-10 col-xs-10 col-xs-offset-1 col-md-offset-1 col-lg-offset-1">
        <form class="form-horizontal" enctype="multipart/form-data" method="post" action="<?= site_url('index.php/doctor/manage_reports'); ?>">
            <div class="form-group" id="data_1">
                <label class="col-lg-2 control-label">Date</label>
                <div class="col-lg-10">
                    <div class="input-group date">
                        <span class="input-group-addon"><i class="fa fa-calendar"></i></span><input type="text" autocomplete="off" name="date" class="form-control" value="<?= isset($_POST['date']) ? $_POST['date'] : $record->date; ?>">
                    </div>
            ...
            <div class="form-group"><label class="col-lg-2 control-label">Type</label>
                <div class="col-lg-10">
                    <select autocomplete="off" data-placeholder="Choose a Country..." class="select chosen-select" name="report_id" tabindex="2">


                        <option value="">Select</option>
                        <option value="operation_report" <?php if (isset($_POST['report_id'])) {
                                                                echo $_POST['report_id'] == 'operation_report' ? 'selected' : '';
                                                            } else {
                                                                echo $record->report_id == 'operation_report' ? 'selected' : '';
                                                            } ?>>Operation</option>

                        <option value="birth_report" <?php if (isset($_POST['report_id'])) {
                                                            echo $_POST['report_id'] == 'birth_report' ? 'selected' : '';
                                                        } else {
                                                            echo $record->report_id == 'birth_report' ? 'selected' : '';
                                                        } ?>>Birth</option>

                        <option value="death_report" <?php if (isset($_POST['report_id'])) {
                                                            echo $_POST['report_id'] == 'death_report' ? 'selected' : '';
                                                        } else {
                                                            echo $record->report_id == 'death_report' ? 'selected' : '';
                                                        } ?>>Death</option>
                    </select>
                    <div class="text-danger d-block text-bold">
                        <p style="padding-left: 2px;padding-top: 2px"><?= form_error('report_id'); ?></p>
                    </div>
                </div>
            </div>
            <div class="form-group"><label class="col-lg-2 control-label">Select Patient</label>
                <div class="col-lg-10">
                    <select autocomplete="off" data-placeholder="Choose a Country..." class="select chosen-select" name="patient_id" tabindex="2">
                        <option value="">Select</option>
                        <?php foreach ($record->patients as $patient) : ?>
                            <option value="<?= $patient->id; ?>" <?php
                                                                    if (isset($_POST['patient_id'])) {
                                                                        echo $_POST['patient_id'] == $patient->id ? 'selected' : '';
                                                                    } else {
                                                                        echo $record->patient_id == $patient->id ? 'selected' : '';
                                                                    }
                    ...
                    <div class="text-danger d-block text-bold">
                        <p style="padding-left: 2px;padding-top: 2px"><?= form_error('patient_id'); ?></p>
                    </div>
                </div>
            </div>
            <div class="form-group"><label class="col-lg-2 control-label">Description</label>
                <div class="col-lg-10">
                    <textarea name="description" class="form-control" rows="5"><?= isset($_POST['description']) ? $_POST['description'] : $record->description; ?></textarea>
                    <div class="text-danger d-block text-bold">
                ...
                <div class="col-lg-10">
                    <div class="fileinput fileinput-new input-group" data-provides="fileinput">
                        ...
                        <span class="input-group-addon btn btn-default btn-file">
                            ...
                            <input type="file" name="custom_report" />
                        </span>
                        <a href="#" class="input-group-addon btn btn-default fileinput-exists" data-dismiss="fileinput">Remove</a>
                    </div>
                    <div class="text-danger d-block text-bold">
                        <p style="padding-left: 2px;padding-top: 2px"><?= isset($upload_error) ? $upload_error : ''; ?></p>
                    </div>
                </div>
            </div>
            <?php if (isset($record->id) || isset($record->hidden_id)) { ?>
                <div class="form-group"><label class="col-lg-2 control-label">Previous Report</label>
                <div class="col-lg-10">

                <input type="text" class="form-control" value="<?= isset($_POST['old_path']) ? $_POST['old_path'] : $record->report_file_path; ?>" disabled>
                <input type="hidden" name="old_path" class="form-control" value="<?= isset($_POST['old_path']) ? $_POST['old_path'] : $record->report_file_path; ?>">
                </div>
            </div>
            <?php } ?>
    </div>
    ...
    <div class="col-lg-offset-2 col-lg-10">
        <?php if (isset($record->id) || isset($record->hidden_id)) { ?>
            <button type="submit" class="btn btn-warning text-white">
                ...
                <i class="fa fa-check"></i>&nbsp;&nbsp;Update Report</button>
