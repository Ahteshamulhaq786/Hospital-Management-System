...
<div style="margin:10px;">
    <a href="<?= base_url('index.php/Patient/appointment_list') ?>">
...
<div class="row animated fadeInDown">
    <div class="col-lg-10 col-md-10 col-xs-10 col-xs-offset-1 col-md-offset-1 col-lg-offset-1">
        <form class="form-horizontal" method="post" action="<?= site_url('index.php/patient/manage_appointment'); ?>">
            <div class="form-group" id="data_1">
                <label class="col-lg-2 control-label">Date</label>
                <div class="col-lg-10">
                    <div class="input-group date">
                        <span class="input-group-addon"><i class="fa fa-calendar"></i></span><input autocomplete="off" required type="text" name="date" class="form-control" value="<?= isset($_POST['date']) ? $_POST['date'] : $record->date ?>">
                    </div>
                    <div class="invalid-feedback d-block text-danger"><?= form_error('date'); ?></div>
                    <?php if(isset($_SESSION['date_error'])): ?>
                        <div class="invalid-feedback d-block text-danger"><?= $_SESSION['date_error'];unset($_SESSION['date_error']); ?></div>
                        <?php endif; ?>
                </div>
            </div>
            <div class="form-group"><label class="col-lg-2 control-label">Time</label>
                <div class="col-lg-10">
                    <div class="input-group">
                        <input required autocomplete="off" name="time" value="<?= isset($_POST['time']) ? $_POST['time'] : $record->time ?>" type="time" class="form-control" name="time">
                        <span class="input-group-addon">
                            <span class="fa fa-clock-o"></span>
                        </span>

                    </div>
                    <div class="invalid-feedback d-block text-danger"><?= form_error('time'); ?></div>
                </div>
            </div>
            <div class="form-group"><label class="col-lg-2 control-label">Select Doctor</label>
                <div class="col-lg-10">
                    <select required autocomplete="off" data-placeholder="Choose a Country..." class="chosen-select" tabindex="2" name="doctor_id">
                        <option value="">Select Doctor</option>
                        <?php foreach ($doctors as $doctor) : ?>
                            <option value="<?= $doctor->id ?>" <?php if (isset($_POST['doctor_id'])) {
                                                                    echo $_POST['doctor_id'] == $doctor->id ? 'selected' : '';
                                                                } else {
                                                                    echo $record->doctor_id == $doctor->id ? 'selected' : '';
                                                                }  ?>><?= $doctor->name; ?></option>
                        <?php endforeach; ?>
                    </select>
                    <div class="invalid-feedback d-block text-danger"><?= form_error('doctor_id'); ?></div>
                </div>
            </div>
            <div class="form-group">
                <div class="col-lg-offset-2 col-lg-10">
                    <button type="submit" class="btn btn-success">
                        <i class="fa fa-check"></i> Request Appointment </button>
                </div>
            </div>
        </form>
    </div>
